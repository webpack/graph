...
Graph.prototype.getColor = function(chunks) {
	var colors = this.colors;
...
Graph.prototype._prepareMaps = function() {
	...
	var modulesList = this.modulesList;
	modulesList.forEach(function(node) {
		var currentDetraction = detractionMap[node.name] = {};
		var currentStrongness = strongnessMap[node.name] = {};
		modulesList.forEach(function(node2) {
			if(node === node2) return;
			currentDetraction[node2.name] = node.getDetraction(node2);
		});
		node.links.forEach(function(node2) {
			currentStrongness[node2.name] = node.getStrongness(node2);
		});
	});
...
Graph.prototype.simulateStep = function() {
	...
	modulesList.forEach(function(node) {
		modulesList.forEach(function(node2) {
			...
			var detraction = this.detractionMap[node.name][node2.name];
		...
		node.links.forEach(function(node2) {
			...
			var strongness = this.strongnessMap[node.name][node2.name];
			linkDistance += this.LINK_DISTANCE_EXTRA / strongness;
			if(distance < VERY_SMALL) {
				node.applyForce([0.5-Math.random(), 0.5-Math.random()], FORCE_LINK * linkDistance * strongness);
			} else {
...
Graph.prototype.normalizePositions = function() {
	...
	var scale = this.scale = Math.min(
		((this.options.width || 1680) - this.MODULE_MAX_R*2) / (max[0] - min[0]),
		((this.options.height || 1050) - this.MODULE_MAX_R*2) / (max[1] - min[1])
	);
	modulesList.forEach(function(node) {
		node.pos[0] -= min[0];
		node.pos[1] -= min[1];
...
Graph.prototype.writeSVG = function(svg) {
	...
	modulesList.forEach(function(node, nodeidx) {
		...
		svgModules.push('<circle id="module',nodeidx,'" class="',node.main?"main":"",'" cx="', screenPos[0], '" cy="', screenPos[1], '" r="',Math.max(this.MODULE_MIN_R, Math.min(this.MODULE_MAX_R, Math.sqrt(node.size)*this.MODULE_SCALE*scale)),'" ');
		svgModules.push('fill="',this._colorName(this.getColor(node.files)),'" onmouseover="a(',nodeidx,',',node.links.length,')" onmouseout="c()">');
		svgModules.push('<title>',this.nameShortener(node.name).split("!").join("\n\n")+'\n\nChunks:\n' + node.files.join("\n"));
		svgModules.push('\n\nlibaryIndex: ' + node.libaryIndex);
